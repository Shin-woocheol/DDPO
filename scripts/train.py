...
def main(_):
    ...
    if config.use_lora:
        ...
        for name in pipeline.unet.attn_processors.keys(): 
            ...
            lora_attn_procs[name] = LoRAAttnProcessor( 
                hidden_size=hidden_size, cross_attention_dim=cross_attention_dim, rank=config.lora_rank, network_alpha=config.lora_rank
            )
    ...
    if config.reward_fn == "hps":
        ...
        ) #* reward function 실행시, dtype/device를 명시해 HPS와 입력 dtype을 일치시킴
    else:
                                ...
                                for n in range(num_repeats):
                                    ...
                                    for i in tqdm(range(0, len(eval_prompts_all), config.sample.batch_size)):
                                        ...
                                        for j, (prompt, image) in enumerate(zip(batch_prompts, images)):
                                            ...
                                            filename = f"{prompt}_{num}_{rewards[j].item():.4f}.png"
